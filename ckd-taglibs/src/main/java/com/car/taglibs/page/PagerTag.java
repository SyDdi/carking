...
public class PagerTag extends TagSupport {
	...
	private static final long serialVersionUID = -8075979570493604241L;
	private String url;         //请求URI

//	private int pageSize = Page.DEFAULT_PAGESIZE;  //每页要显示的记录数
//	private int currentPageIndex = Page.DEFAULT_CURRENT_PAGE_INDEX;     //当前页号

    //需要修改这里  原先是从车王那里拿过来的，尚未来得及修改？
	...
	public int doStartTag() throws JspException {
		...
		if(totalSize == 0){
			...
			sb.append("<form method=\"get\" action=\"").append(this.url).append("\" name=\"qPagerForm\">\r\n");
			//获取请求中的所有参数
			...
			while (enumeration.hasMoreElements()) {
				...
				// 去除页号
				if (name.equals("currentPageIndex")) {
					if (null != value && !"".equals(value)) {
						currentPageIndex = Integer.parseInt(value);
					}
					continue;
				}
			...
			// 把当前页号设置成请求参数
			sb.append("<input type=\"hidden\" name=\"").append("currentPageIndex").append("\" value=\"").append(currentPageIndex).append("\"/>\r\n");
			// 输出统计数据
			sb.append("&nbsp;共<strong>").append(totalSize)
			  .append("</strong>项")
			  .append(",<strong>")
			  .append(pageCount)
			  .append("</strong>页:&nbsp;\r\n");
			...
			sb.append("    document.qPagerForm.currentPageIndex.value=no;\r\n");
